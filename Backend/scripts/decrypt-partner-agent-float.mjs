 ...
 *
 *   node scripts/decrypt-partner-agent-float.mjs \
 *     --body ./captured-request.json \
 *     --signature "sha256=abc123..."
 *
 *   # Decrypt only (skip signature check — not for production)
 *   node scripts/decrypt-partner-agent-float.mjs --body ./captured-request.json --skip-signature
 *
 *   # Print first 10 agents only
 *   node scripts/decrypt-partner-agent-float.mjs --body ./captured-request.json --skip-signature --limit 10
 */
...
function main() {
  ...
  if (!opts.skipSignature) {
    ...
    if (!ok) {
      ...
      console.error(
        'Use the exact raw POST body bytes (no re-formatting) and the correct HMAC secret.'
      );
      process.exit(1);
    }
  ...
  if (opts.summary) {
    const totalBalance = inner.agents.reduce(
      (sum, row) => sum + Number(row.after_balance || 0),
      0
    );
